...
TPCW_SRC=applications/tpc-w-fenix
...
TXMUD_LIBPATH=/tmp/$USER/txmud/dist/jars
...
function deploy_mysql_driver(){
	...
	wget -P /tmp -c http://ftp.gwdg.de/pub/misc/mysql/Downloads/Connector-J/mysql-connector-java-5.1.17.zip
	echo "Done"
	echo "----------------------------"
	echo "Deploying mysql driver on tomcat lib dir "
	mkdir -p $TOMCAT_LIB
	DIR_=`pwd`
	cd /tmp && unzip mysql-connector-java-5.1.17.zip > /dev/null
	mv /tmp/mysql-connector-java-5.1.17/mysql-connector-java-5.1.17-bin.jar $TOMCAT_LIB
	rm -rf /tmp/mysql-connector-java-5.1.17
	cd $DIR_
	echo "Done"
	echo "----------------------------"
}
...
function deploy_storage(){
	...
	wget -P /tmp -c http://vesta.informatik.rwth-aachen.de/mysql/Downloads/MySQL-5.5/mysql-5.5.15.tar.gz
	CURR_DIR=`pwd`
	echo "Unpacking mysql source"
	tar xzvf /tmp/mysql-5.5.15.tar.gz   -C /tmp > /dev/null
	echo "Configuring  mysql source"
	cd /tmp/mysql-5.5.15 && cmake -DCMAKE_INSTALL_PREFIX=${MYSQL_ROOT} -DMYSQL_DATADIR=${MYSQL_ROOT}/data -DWITH_DEBUG=ON > /dev/null 
	...
	cd ${MYSQL_ROOT} && scripts/mysql_install_db --defaults-file=${MYSQL_ROOT}/mysql-test/include/default_mysqld.cnf \
												    ...
												    --user=$USER
	#cd ${MYSQL_ROOT} && scripts/mysql_install_db --defaults-file=${MYSQL_ROOT}/support-files/my-huge.cnf \
	#											    --basedir=${MYSQL_ROOT} \
	#											    --datadir=${MYSQL_ROOT}/data \
	#											    --user=$USER
	...
	#${MYSQL_ROOT}/bin/mysqladmin --defaults-file=${MYSQL_ROOT}/support-files/my-huge.cnf -u root password '101010';
	echo ""	
	...
	#${MYSQL_ROOT}/bin/mysql --defaults-file=${MYSQL_ROOT}/support-files/my-huge.cnf \
	#				   -u root --password='101010' -e	"grant all on *.* to root@'%' identified by '101010' with grant option"
	echo ""
	sleep 2
	...
	#${MYSQL_ROOT}/bin/mysqladmin --defaults-file=${MYSQL_ROOT}/support-files/my-huge.cnf -u root --password='101010' flush-privileges;
...
CMD="${MYSQL_ROOT}/bin/mysqld_safe --defaults-file=${MYSQL_ROOT}/mysql-test/include/default_mysqld.cnf --port=$MYSQL_PORT \
...
#best match
#CMD="${MYSQL_ROOT}/bin/mysqld_safe --defaults-file=${MYSQL_ROOT}/mysql-test/include/default_mysqld.cnf --port=$MYSQL_PORT \
#	 						--innodb_lock_wait_timeout=1 --max_connections=10000 --max_user_connections=10000 --innodb_buffer_pool_size=2G \
#	 						--innodb_flush_method=O_DIRECT --skip-innodb_doublewrite --innodb_flush_log_at_trx_commit=0 \
#							--query_cache_size=512M --innodb_thread_concurrency=12 --innodb_file_per_table"
#original
...
function deploy_webproxy(){
	...
	wget -P /tmp -c http://www.bitlib.net/mirror/apache.org/tomcat/tomcat-6/v6.0.33/bin/apache-tomcat-6.0.33.tar.gz
	echo "Done"
	echo "----------------------------"
	echo "Installing Tomcat6"
	tar xzvf /tmp/apache-tomcat-6.0.33.tar.gz -C /var/tmp/$USER/txmud/  > /dev/null
