...
public class TPCW_Database {
    ...
    public static Connection getNewConnection() {
		...
		System.err.println("INFO: configuring scratchpad: actualDriver <"+actualDriver+"> jdbcActualpath <"+
				jdbcActualPath+"> username:<"+user+"> password:<"+password+"> padClass: <"+padClass+">");
		//scratchpad configuration
		ScratchpadConfig config = new ScratchpadConfig(actualDriver,jdbcActualPath,user,password,padClass);
		try { DBScratchpad.prepareDBScratchpad(config);	} 
		catch (ScratchpadException e) {	e.printStackTrace();}
	...
	try {
		...
		try {
		    ...
		    txFinished = true;
		 	...
		 	if( time > startmi && time < endmi )	
				aborts.incrementAndGet();
		    System.err.println("Restarting TX because of a database problem (hopefully just a conflict) total aborts within the MI so far:"+aborts);		    spde.printStackTrace();
		    con.rollback();
		    returnConnection(con);
		    txFinished = true;
		} 
	...
	int SHOPPING_ID;;
		...
		Calendar calendar = Calendar.getInstance();
		insert_cart.setDate(2, new java.sql.Date(calendar.getTimeInMillis()));
	...
	if(cc_name.length() > 30)
	    ...
	    statement.setDate(7, new java.sql.Date(System.currentTimeMillis()));     // cx_xact_date
