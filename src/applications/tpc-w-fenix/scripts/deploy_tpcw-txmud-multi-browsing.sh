...
TPCW_THINKTIME=0.0
BACKEND="txmud_ssc" #options: mysql, txmud, txmud_ssc, scratchpad
RESULTFILE_TAG="TxMudRedBlueSSC"  #it should be defined according to table color and the backend: TxMudAllBlue,TxMudRedBlue,Mysql
...
###############AMAZON###############
####DATACENTER 0
TXM_DATABASE[0]="ec2-72-44-40-40.compute-1.amazonaws.com"
TXM_PROXY[0]="ec2-184-72-132-14.compute-1.amazonaws.com"
TXM_COORDINATOR[0]="ec2-107-20-82-151.compute-1.amazonaws.com"
TXM_STORAGESHIM[0]="ec2-184-73-21-123.compute-1.amazonaws.com"
TXM_USER[0]="ec2-184-72-145-96.compute-1.amazonaws.com"
####DATACENTER 1
TXM_DATABASE[1]="ec2-46-137-138-162.eu-west-1.compute.amazonaws.com"
TXM_PROXY[1]="ec2-46-51-150-50.eu-west-1.compute.amazonaws.com"
TXM_COORDINATOR[1]="ec2-79-125-88-188.eu-west-1.compute.amazonaws.com"
TXM_STORAGESHIM[1]="ec2-79-125-76-191.eu-west-1.compute.amazonaws.com"
TXM_USER[1]="ec2-79-125-73-105.eu-west-1.compute.amazonaws.com"
############THOR MPI################
####DATACENTER 0
...
if [ -n $2 ]
	then
		for((i=0;i<$DATACENTERNUM;i++)) do
			CMD_START_EXPERIMENT[$i]="cd $TPCW_BINARIES && ./rbe.sh -w http://${TXM_PROXY[$i]}:8080/tpcw/ -b $RESULTFILE_TAG  	-l $i -t $TPCW_MIX -u $TPCW_WARMUPTIME  -d $TPCW_TEARDOWNTIME -i $TPCW_MEASUREMENTINTERVAL -n $2"
			CMD_WAIT_USERS[$i]="while  [ \`ps aux | grep $USERNAME | grep java | grep RBE | grep -v grep | wc -l\`  -ne 0 ]; do  echo 'waiting...' && sleep 10 ;  done"
		done;
else
	for ((i=0;i<$DATACENTERNUM;i++)) do
			CMD_START_EXPERIMENT[$i]="cd $TPCW_BINARIES && ./rbe.sh -w http://${TXM_PROXY[$i]}:8080/tpcw/ -b $RESULTFILE_TAG  	-l $i -t $TPCW_MIX -u $TPCW_WARMUPTIME  -d $TPCW_TEARDOWNTIME -i $TPCW_MEASUREMENTINTERVAL -n 100"
			CMD_WAIT_USERS[$i]="while  [ \`ps aux | grep $USERNAME | grep java | grep RBE | grep -v grep | wc -l\`  -ne 0 ]; do  echo 'waiting...' && sleep 10 ;  done"
		done;
fi
...
configure(){
	...
	for((i=0;i<$DATACENTERNUM;i++)) do
		...
		$CMD_SCP  ${CREDENTIAL[$i]} $TOPOLOGYXMLFILENAME  $DATABASEXMLFILENAME ${USERNAME}"@"${TXM_USER[$i]}":"${TPCW_ROOT}
...
update_src(){
	for((i=0;i<$DATABASENUM;i++)) do
		...
		python tpcwtxmud_remote_setup.py -d ${TXM_STORAGESHIM[$i]}  -u $USERNAME $AUTH_MODE "${CREDENTIAL[$i]}" -r mshim --run "cd $TXMUD_ROOT && svn up"
		python tpcwtxmud_remote_setup.py -d ${TXM_COORDINATOR[$i]}  -u $USERNAME $AUTH_MODE "${CREDENTIAL[$i]}" -r coordinator --run "cd $TXMUD_ROOT && svn up"
		python tpcwtxmud_remote_setup.py -d ${TXM_PROXY[$i]}  -u $USERNAME $AUTH_MODE "${CREDENTIAL[$i]}" -r proxy --run "cd $TXMUD_ROOT && svn up"
		python tpcwtxmud_remote_setup.py -d ${TXM_USER[$i]}  -u $USERNAME $AUTH_MODE "${CREDENTIAL[$i]}" -r user --run "cd $TXMUD_ROOT && svn up"
	done;
}
start_users(){
