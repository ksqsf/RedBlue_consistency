...
MYSQL_PORT=53306
MYSQL_PATH=/var/tmp/$USER/mysql-5.5.18/bin/
MYSQL_ROOT=/var/tmp/$USER/mysql-5.5.18
...
TXMUD_DEFAULT_LOGICAL_CLOCK=0-0-0
#TPCW_SRC=/root/devel/txmud/src/applications/tpc-w-fenix
TPCW_SRC=/home/$USER/java/src/applications/tpc-w-fenix
TXMUD_LIBPATH=/var/tmp/$USER/output/txmud/dist/jars
#TXMUD_SRC=/root/devel/txmud
TXMUD_SRC=/home/$USER/java
##########################################################################
# Databases
##########################################################################
# scratchpad
function deploy_txmud_components(){
	...
	rm -f  $TOMCAT_LIB/jsqlparser.jar	$TOMCAT_LIB/netty-3.2.1.Final.jar	$TOMCAT_LIB/log4j-1.2.15.jar $TOMCAT_LIB/jdbctxmud.jar 
	...
	cp $TXMUD_SRC/lib/jsqlparser.jar $TOMCAT_LIB
	cp $TXMUD_SRC/lib/netty-3.2.1.Final.jar $TOMCAT_LIB
	cp $TXMUD_SRC/lib/log4j-1.2.15.jar $TOMCAT_LIB
	cd $PDIR
	echo "Done"
	echo "----------------------------"
...
function deploy_storage(){
	...
	wget -P /var/tmp/$USER -c http://vesta.informatik.rwth-aachen.de/mysql/Downloads/MySQL-5.5/mysql-5.5.18.tar.gz
	CURR_DIR=`pwd`
	mkdir -p /var/tmp/$USER/output
	echo "Unpacking mysql source"
	tar xzvf /var/tmp/$USER/mysql-5.5.18.tar.gz   -C /var/tmp/$USER/output > /dev/null
	echo "Configuring  mysql source"
	echo "cd /var/tmp/$USER/output/mysql-5.5.18 && cmake -DCMAKE_INSTALL_PREFIX=${MYSQL_ROOT} -DMYSQL_DATADIR=${MYSQL_ROOT}/data -DENABLED_PROFILING=OFF > /tmp/mysql-install.output"
	cd /var/tmp/$USER/output/mysql-5.5.18 && cmake -DCMAKE_INSTALL_PREFIX=${MYSQL_ROOT} -DMYSQL_DATADIR=${MYSQL_ROOT}/data -DENABLED_PROFILING=OFF > /tmp/mysql-install.output 
...
function undeploy_storage(){
	stop_database;
	rm -rf /var/tmp/$USER/mysql-5.5.18
}
...
function setupDB_mysql_txmud_sharedcart() {
	echo "------------------------------------------------------------"
	echo "Setting up mysql_txmud_sharedcart"
	#$MYSQL_CMD  -e "drop database if exists mysql_txmud_sharedcart";
	$MYSQL_CMD  -e "create database mysql_txmud_sharedcart";
	$MYSQL_CMD  -e "grant all on mysql_txmud_sharedcart.* to sa@'%'";
	$MYSQL_CMD  -e "grant all on mysql_txmud_sharedcart.* to sa@'localhost'";
	$MYSQL_CMD  -e "grant all on mysql_txmud_sharedcart.* to sa";
}
...
function tpcw_create_databases() {
	...
	setupDB_mysql_txmud_sharedcart;
}
#------------------------------------------------------------------------
# Populate data
#-----------------------------------------------------------------------
...
echo "------------------------------------------------------------"
	...
	then
		...
		ANT_CMD="$ANT_CMD -Dbackend=$1 -Dmysql_host=$2 -Dmysql_port=$3 -Dnum.eb=$4 -Dnum.item=$5 -Dlogicalclock=${TXMUD_DEFAULT_LOGICAL_CLOCK}"
	fi
