...
if [ -n $2 ]
	then
		for((i=0;i<$DATACENTERNUM;i++)) do
		    for((j=0;j<$USERNUM;j++)) do
				CMD_START_EXPERIMENT[$j]="cd $TPCW_BINARIES && ./rbe.sh -w http://${TXM_PROXY_0[$j]}:8080/tpcw/ -b $RESULTFILE_TAG  	-l $i -m $j -t $TPCW_MIX -u $TPCW_WARMUPTIME  -d $TPCW_TEARDOWNTIME -i $TPCW_MEASUREMENTINTERVAL -n $2"
	...
	for ((i=0;i<$DATACENTERNUM;i++)) do
		for((j=0;j<$USERNUM;j++)) do
			CMD_START_EXPERIMENT[$j]="cd $TPCW_BINARIES && ./rbe.sh -w http://${TXM_PROXY_0[$j]}:8080/tpcw/ -b $RESULTFILE_TAG  	-l $i -m $j -t $TPCW_MIX -u $TPCW_WARMUPTIME  -d $TPCW_TEARDOWNTIME -i $TPCW_MEASUREMENTINTERVAL -n 100"
...
configure(){
	...
	for((i=0;i<$DATACENTERNUM;i++)) do
		for((j=0;j<$USERNUM;j++)) do
			...
			python tpcwtxmud_remote_setup.py -d ${TXM_USER_0[$j]} -u $USERNAME $AUTH_MODE "${CREDENTIAL[$i]}" --run "${CMD_CONFIGURE[$j]}"
		done;
		for((j=0;j<$PROXYNUM;j++)) do
