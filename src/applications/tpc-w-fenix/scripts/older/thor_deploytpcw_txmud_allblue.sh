...
USERNAME="dcfp"
AUTH_MODE="-p"
CREDENTIAL="mk38qa9t@mp"
#AUTH_MODE="-k"
#CREDENTIAL="/home/dcfp/EU/tpcw-kf.pem"
TXM_DATABASE="139.19.158.74"  
TXM_PROXY="139.19.158.70"
TXM_USER="139.19.158.72"
TXM_COORDINATOR="139.19.158.68"
TXM_STORAGESHIM="139.19.158.69"
...
POPULATE="$TPCW_DIR/tpcwtxmud_setup-original.sh populate $BACKEND localhost  53306 10 1000" 
...
START_STORAGESHIM="java -jar $TXMUD_JARS/storageshim-big.jar $TPCW_DIR/$TOPOLOGYXMLFILENAME $TPCW_DIR/$DATABASEXMLFILENAME 0 0 20 true 100 2>&1 | tee /tmp/s.out "

#Coordinator config.xml dcId coordinatorId threadCount tokensize tcpnodelay blueTimeOut
START_COORDINATOR="java -jar $TXMUD_JARS/coordinator-big.jar $TPCW_DIR/$TOPOLOGYXMLFILENAME 0 0 20 10000000 true 1000000 2>&1 |tee /tmp/c.out "
#reset logical clock
...
if [ $1 = "install_all" ];	
	then 
	python tpcwtxmud_remote_setup.py -d $TXM_DATABASE  -u $USERNAME $AUTH_MODE "$CREDENTIAL" --run "$TPCW_DIR/tpcwtxmud_setup-original.sh install_proxy"
	python tpcwtxmud_remote_setup.py -d $TXM_DATABASE  -u $USERNAME $AUTH_MODE "$CREDENTIAL" --run "$TPCW_DIR/tpcwtxmud_setup-original.sh install_database" 
	python tpcwtxmud_remote_setup.py -d $TXM_PROXY  -u $USERNAME $AUTH_MODE "$CREDENTIAL" --run "$TPCW_DIR/tpcwtxmud_setup-original.sh install_proxy"
	python tpcwtxmud_remote_setup.py -d $TXM_USER  -u $USERNAME $AUTH_MODE "$CREDENTIAL" --run "$TPCW_DIR/tpcwtxmud_setup-original.sh install_proxy"
	python tpcwtxmud_remote_setup.py -d $TXM_COORDINATOR  -u $USERNAME $AUTH_MODE "$CREDENTIAL" --run "$TPCW_DIR/tpcwtxmud_setup-original.sh install_proxy"
	python tpcwtxmud_remote_setup.py -d $TXM_STORAGESHIM  -u $USERNAME $AUTH_MODE "$CREDENTIAL" --run "$TPCW_DIR/tpcwtxmud_setup-original.sh install_proxy"
	
elif [ $1 = "get" ];	
	then 
	local_results="results_$TXM_USER"
	mkdir -p $local_results
	scp -i ${CREDENTIAL} -r ${USERNAME}"@"${TXM_USER}":"${TPCW_BIN}"/results/*" ${local_results}
	scp -i ${CREDENTIAL} -r ${USERNAME}"@"${TXM_COORDINATOR}":/tmp/c.out" ${local_results}
	scp -i ${CREDENTIAL} -r ${USERNAME}"@"${TXM_STORAGESHIM}":/tmp/s.out" ${local_results}
	...
	python tpcwtxmud_remote_setup.py -d $TXM_STORAGESHIM  -u $USERNAME $AUTH_MODE "$CREDENTIAL" --run "$PREPARE"
	python tpcwtxmud_remote_setup.py -d $TXM_COORDINATOR  -u $USERNAME $AUTH_MODE "$CREDENTIAL" --run "$PREPARE"
	...
	then	
	python tpcwtxmud_remote_setup.py -d $TXM_DATABASE  -u $USERNAME $AUTH_MODE "$CREDENTIAL" --run "$TPCW_DIR/tpcwtxmud_setup-original.sh reset_lc"
elif [ $1 = "reset" ];
	then
	...
	then
	python tpcwtxmud_remote_setup.py -d $TXM_STORAGESHIM  -u $USERNAME $AUTH_MODE "$CREDENTIAL" -r mshim --run "cd $TPCW_DIR && svn up"
	python tpcwtxmud_remote_setup.py -d $TXM_COORDINATOR  -u $USERNAME $AUTH_MODE "$CREDENTIAL" -r coordinator --run "cd $TPCW_DIR && svn up"
	python tpcwtxmud_remote_setup.py -d $TXM_DATABASE  -u $USERNAME $AUTH_MODE "$CREDENTIAL" -r storage --run "cd $TPCW_DIR && svn up"
	python tpcwtxmud_remote_setup.py -d $TXM_PROXY  -u $USERNAME $AUTH_MODE "$CREDENTIAL" -r proxy --run "cd $TPCW_DIR && svn up"
	python tpcwtxmud_remote_setup.py -d $TXM_USER  -u $USERNAME $AUTH_MODE "$CREDENTIAL" -r user --run "cd $TPCW_DIR && svn up"
