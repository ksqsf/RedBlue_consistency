 ...
 */
    ...
    throws IOException, ServletException
  {
    ...
    try
    {
      ...
      int newBidId = Config.BidIDFactory.addAndGet(Config.TotalProxies);
      stmt =
      ...
      try
      {
        ...
        else
        {
            ...
            try{
        	    DBRUBISShdStoreBid3 dSB3 = DBRUBISShdStoreBid3.createOperation(newBidId, userId, 
        	            itemId, qty, bid, maxBid, now);
        	    conn.setShadowOperation(dSB3, 0);
            }catch (IOException e){
      ...
      if(isGreater){
      try{
  	    DBRUBISShdStoreBid1 dSB1 = DBRUBISShdStoreBid1.createOperation(newBidId, userId, 
  	            itemId, qty, bid, maxBid, now);
  	    conn.setShadowOperation(dSB1, 0);
      }catch (IOException e){
  		// TODO Auto-generated catch block
  		e.printStackTrace();
      }
          ...
          try{
        	    DBRUBISShdStoreBid2 dSB2 = DBRUBISShdStoreBid2.createOperation(newBidId, userId, 
        	            itemId, qty, bid, maxBid, now);
        	    conn.setShadowOperation(dSB2, 0);
            }catch (IOException e){
        		// TODO Auto-generated catch block
        		e.printStackTrace();
            } 
