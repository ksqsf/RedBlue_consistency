...
public class UserSession extends Thread
{
  ...
  public URL computeURLFromState(int state)
  {
    ...
    switch (state)
    {
      ...
      case 27:
    	  //get auctions for close page
    	 URL getCloseUrl = urlGen.getAuctionsReadyForClose(rubis.getMaxOpenItemId());
    	 //System.out.println("user " + this.userId +" getClose " + getCloseUrl.toString());
    	return getCloseUrl;
      case 28:
      {
    	  //close auction page
    	int itemId = extractItemIdFromHTML();
        if (itemId == -1){
        	URL previousUrl = computeURLFromState(transition.backToPreviousState());
        	 //System.out.println("user " + this.userId +" closeReturnPreivous " + previousUrl.toString());
	      return previousUrl; // Nothing
        }
        // then go
        // back
        else{
        	URL closeUrl = urlGen.closeAuction(itemId);
        	//System.out.println("user " + this.userId +" closeUrl " + closeUrl.toString());
          return closeUrl;
        }
      }
      default :
