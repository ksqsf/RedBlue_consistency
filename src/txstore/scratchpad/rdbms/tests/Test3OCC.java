...
public class Test3OCC
{
	...
	public static void main( String[] args) {
		try {
			...
			rq = (DBSelectResult)db.execute( new DBSingleOperation( "select * from t2 where a > 100 limit 1;"));
			Debug.println( "query result = \n" + rq);
			
			if( rq.next()) {
				int a = rq.getInt(1);
				ru =  (DBUpdateResult)db.execute( new DBSingleOperation( "update t2 set c = c + 10 where a =" + a + ";"));
				Debug.println( "result = " + ru);
				ru =  (DBUpdateResult)db2.execute( new DBSingleOperation( "update t2 set d = d + 50 where a =" + a + ";"));
				Debug.println( "result = " + ru);
			}
