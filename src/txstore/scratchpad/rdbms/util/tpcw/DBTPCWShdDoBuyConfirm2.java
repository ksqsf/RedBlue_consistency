...
/**
 * Blue transaction, decreased + new address
 * 
 * @author chengli
 *
 */
public class DBTPCWShdDoBuyConfirm2 extends DBShadowOperation {
	int shopping_id;
	int customer_id;
	...
	public static DBTPCWShdDoBuyConfirm2 createOperation(
		...
		return new DBTPCWShdDoBuyConfirm2(baos.toByteArray(), shopping_id, customer_id, 
				addr_street1, addr_street2, addr_city, addr_state, addr_zip, country_id,
	...
	public static DBTPCWShdDoBuyConfirm2 createOperation(		int shopping_id, int customer_id, 
			String addr_street1,	String addr_street2,	String addr_city, String addr_state,
			String addr_zip, int country_id,
	...
	protected DBTPCWShdDoBuyConfirm2(byte[] arr, 	int shopping_id, int customer_id, 
			String addr_street1,	String addr_street2,	String addr_city, String addr_state,
			String addr_zip, int country_id,
			int o_id, long o_date, double sc_sub_total, double sc_total, long o_ship_date, int customer_addr_id, int ship_addr_id,
			Vector<Integer> it_id_v, Vector<Integer> it_q_v, Vector<String> it_c_v, Vector<Integer> it_s_v, String cc_type,
	...
	public DBTPCWShdDoBuyConfirm2(byte[] arr) {
		super(arr);
	...
	public void executeShadow(IDefDatabase iDefDatabase) {
		// TODO Auto-generated method stub
		Debug.println("execute shadow doBuyConfirm decrease + address");
		//insert address record
		try {
			String insertAddrQuery = "INSERT into address (addr_id, addr_street1, addr_street2, addr_city, addr_state, addr_zip, addr_co_id) VALUES ( "+
			this.ship_addr_id +", '"+this.addr_street1+"', '"+this.addr_street2+"', '"+
			this.addr_city+"', '"+this.addr_state+"', '"+this.addr_zip+"', "+this.country_id+")";
			Debug.println(insertAddrQuery);
			iDefDatabase.executeUpdate(insertAddrQuery);
		} catch (Exception e) {
			System.err
		...
		for(int i = 0; i < this.it_id_v.size(); i++){
		 ...
		 "VALUES ("+i+", "+this.o_id+", "+this.it_id_v.get(i).intValue()+", "+this.it_q_v.get(i).intValue()+", "+this.c_discount+", '"+this.it_c_v.get(i)+"')";
			...
			Debug.println(orderlineQuery);
			Debug.println(stockQuery);
			try {
