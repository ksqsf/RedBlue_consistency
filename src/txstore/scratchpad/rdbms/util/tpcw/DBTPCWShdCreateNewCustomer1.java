...
public class DBTPCWShdCreateNewCustomer1 extends DBShadowOperation {
	...
	long c_login, long c_expiration, long c_birthdate, String c_data, int addr_id,
		...
		/*String inputStr = "createNewCustomer1 " + c_id + "-" + c_uname +"-"+ c_passwd + "-" + c_discount;
		inputStr += "-"+ c_fname + "-" + c_lname +"-"+ c_phone + "-" + c_email +"-" + c_last_visit +"-"+ c_since;
		inputStr += "-" + c_login + "-" +c_expiration +"-"+ c_birthdate +"-"+ c_data +"-"+ addr_id;
		inputStr += "-"+ street1 +"-"+ street2 +"-"+ city +"-"+ state +"-"+ zip +"-"+ country_id;
