	...
	implements ScratchpadInterface, IDBScratchpad
{
			...
			if( dbOp0 instanceof DBShadowOperation){
				...
				dbShadowOp.executeShadow( new IDefDatabase() {
					...
					public int executeOp(String sql)throws SQLException, ScratchpadException{
						...
						if( tableName.length == 1) {
							...
							if( op.isQuery())
								...
								Debug.println("execute update in policy " + sql);
								policy.executeOnlyOp( op, mainDB, lc, ts, ! deletedTables.contains(tableName[0][2]));
							}
						}else 
					...
					public int executeUpdate(String sql) throws SQLException, ScratchpadException {
						try {
							Debug.println("execute update " + sql);
							DBSingleOperation op = new DBSingleOperation( sql);
							op.parseSQL(parser);
							String[][] tableName = op.targetTable();
							if( tableName.length == 1) {
	...
	/* (non-Javadoc)
	 * @see txstore.scratchpad.rdbms.IDBScratchpad#executeOp(java.lang.String)
	 */
