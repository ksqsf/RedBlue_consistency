// $Id: UnsignedTypes.java 510 2011-07-04 11:01:22Z chengli $
...
public class UnsignedTypes{
 ...
 /** 
    ...
    public static void printHash(byte []hash){
        Debug.println(bytesToHexString(hash));
    }
	...
	//Debug.println("i\tint\t\tb1\tb2\t\tint(b(i))\tlong\t\tb1\tb2\tb3\tb4\t\tlong(b(l))");
	...
	//Debug.println("\t"+bytesToLong(longToBytes(longval)));
	intval++;
	longval++;
	Debug.println();

	Debug.println("testing long long");
	long i = 2;
	for (; i > 0; i = (i-1)*2+1)
	    Debug.println(i+" : " + bytesToString(longlongToBytes(i))+ " : "+ bytesToLongLong(longlongToBytes(i)));
	
	Debug.println("testing  long");
	long j = 2;
	for (; ((int)j) > 0; j = (j-1)*2+1)
	    Debug.println(j+" : " + bytesToString(longToBytes(j))+ " : "+ bytesToLong(longToBytes(j)));
	
	Debug.println("testing  int");
	int k = 2;
	for (; ((short)k) > 0; k = (k-1)*2+1)
	    Debug.println(k+" : " + bytesToString(intToBytes(k))+ " : "+ bytesToInt(intToBytes(k)));
